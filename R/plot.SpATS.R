plot.SpATS <-
function(x, all.in.one = TRUE, main = NULL, annotated = FALSE, ...) {
	...
	df <- data.table(columns = x.coord, rows = y.coord, response = response, fitted = fitted, residuals = residuals, geno.pred = geno.pred, ONE = ONE)
